...
export class ManualIDVerifier {
    private storage: any;
    public numTicketsUpdated: number = 0;

    constructor(
        private env: any,
        private db: any) {

        let google_cloud_storage = require('@google-cloud/storage')({
            projectId: this.env.FIREBASE_PROJECT_ID,
            keyFilename: `serviceAccountCredentials.${this.env.FIREBASE_PROJECT_ID}.json`,
        });
        this.storage = google_cloud_storage.bucket(`${this.env.FIREBASE_PROJECT_ID}.appspot.com`);
    }


    openFreshdeskTicketIfNecessary(userID: string): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.freshdeskMarkup(user)
                ...
                .then((markup: string) => {
                    ...
                    let URL = "https://" + this.env.FRESHDESK_DOMAIN + ".freshdesk.com" + PATH;
                    ...
                    Request.auth({
                        user: this.env.FRESHDESK_API_KEY,
                        pass: "X",
...
<p><a href="${this.appLink({ redirect: 'user', id: user.userId, approve: true })}" target="_blank">Approve</a> | <a href="${this.appLink({ redirect: 'user', id: user.userId })}" target="_blank">View record</a></p>
    ...
    private appLink(args: any): string {
        ...
        let str = this.env.APP_BASE_URL + '?admin-redirect=true';
